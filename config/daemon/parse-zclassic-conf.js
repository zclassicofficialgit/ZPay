...
export const parseZclassicConf = (customDir: ?string): Promise<ZclassicConfFile> => new Promise((resolve, reject) => {
  fs.readFile(customDir || locateZclassicConf(), (err, file) => {
    ...
    // $FlowFixMe
    const payload: ZclassicConfFile = filterObjectNullKeys(
      fileString.split('\n').reduce((acc, cur) => {
        if (!cur) return acc;
...
export const generateArgsFromConf = (obj: ZclassicConfFile): Array<string> => Object.keys(obj).reduce((acc, key) => {
