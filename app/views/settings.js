...
const RUNNING_NON_EMBEDDED_DAEMON_WARNING = 'You are using a separate zclassicd process, in order to change the network, you need to restart the process yourself';
...
export class SettingsView extends PureComponent<Props, State> {
  ...
  backupWalletDat = async () => {
    const backupFileName = `zclassic-wallet-backup-${dateFns.format(
      new Date(),
      'YYYY-MM-DD-mm-ss',
    )}.dat`;
